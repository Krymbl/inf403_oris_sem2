...
public class PathScan {
    ...
    public static List<Class<?>> findByAnnotation(String scannedPackage, Class<? extends Annotation> annotation) {
        ...
        //Проходит по всем файлам и папкам внутри целевой папки и вызывает метод findByAnnotation для каждого.
        for (File file : scannedDir.listFiles()) {
            classes.addAll(findByAnnotation(file, scannedPackage, annotation));
        }
        return classes;
    }

    private static List<Class<?>> findByAnnotation(File file, String scannedPackage, Class<? extends Annotation> annotation) {
        List<Class<?>> classes = new ArrayList<>();
        ...
        if (file.isDirectory()) {
            for (File child : file.listFiles()) {
                classes.addAll(findByAnnotation(child, resource, annotation));
            }
